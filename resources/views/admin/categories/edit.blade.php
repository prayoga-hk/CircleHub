...
<head>
    ...
    <title>Edit Kategori - CircleHub</title>
...
<body class="bg-[#0b1120] text-white font-sans min-h-screen flex items-center justify-center p-4">
    ...
    <div class="w-full max-w-lg bg-[#161f33] p-8 rounded-2xl border border-slate-800 shadow-xl">
        <h2 class="text-xl font-bold mb-1">Edit Kategori</h2>
        <p class="text-xs text-slate-400 mb-6">Ubah data kategori <span class="text-[#6C5CE7] font-semibold">{{ $category->name }}</span></p>
        ...
        <form action="{{ route('categories.update', $category->id) }}" method="POST" class="space-y-4">
            ...
            <div>
                ...
                <textarea name="description" rows="3"
                          class="w-full px-3.5 py-2.5 bg-[#0b1120] text-white rounded-xl border border-slate-700/70 focus:outline-none focus:border-[#6C5CE7] focus:ring-1 focus:ring-[#6C5CE7] text-sm">{{ old('description', $category->description) }}</textarea>
            </div>
            ...
            <div class="flex items-center justify-end gap-3 pt-2">
                ...
                <button type="submit" class="px-5 py-2.5 bg-[#6C5CE7] hover:bg-[#5b4bc4] text-white text-xs font-semibold rounded-xl transition shadow-md shadow-[#6C5CE7]/20">Update Kategori</button>
            </div>
        </form>
    </div>
